            ...
            <div class="col" style="flex:1;">
                <div style="position:sticky;top:120px;">
                    <img src="{{$product->image}}" alt="{{$product->name}}"
                        style="width:100%;height:auto;aspect-ratio:1;object-fit:cover;border-radius:16px;box-shadow:var(--shadow-md);">
                </div>
            </div>
            <div class="col" style="flex:1;">
                <span class="badge badgeSuccess"
                    style="display:inline-block;margin-bottom:1rem;">{{$product->category->name}}</span>
                ...
                <div style="display:flex;align-items:baseline;gap:1rem;margin-bottom:2rem;">
                    ...
                    <span
                        style="text-decoration:line-through;color:var(--text-light);font-size:1.1rem;">{{number_format($product->price * 1.2)}}
                        đ</span>
                </div>
                <div style="background:var(--bg-body);padding:1.5rem;border-radius:12px;margin-bottom:2rem;">
                    <p style="margin:0;color:var(--text-light);line-height:1.6;">
                        {{$product->description ?? 'Trải Nghiệm Sự Tinh Tế Và Đẳng Cấp Với Thiết Kế Này! Chất Liệu Cao Cấp Được Tuyển Chọn Kỹ Lưỡng, Mang Lại Cảm Giác Thoải Mái Tuyệt Đối Cho Người Mặc!'}}
                    </p>
                </div>
                @if($product->stock < 10)
                    <div
                        style="background:#FFF8E1;border:1px solid #F57F17;border-radius:8px;padding:1rem;margin-bottom:1.5rem;">
                        ...
                        <div style="display:flex;justify-content:space-between;align-items:center;">
                            ...
                            <div class="quantityControl">
                                <button type="button" class="qtyBtn"
                                    onclick="if(document.getElementById('qty').value>1)document.getElementById('qty').value--">-</button>
                                <input type="number" name="quantity" id="qty" value="1" min="1" max="{{$product->stock}}"
                                    class="qtyInput" style="width:60px;text-align:center;">
                                <button type="button" class="qtyBtn"
                                    onclick="if(document.getElementById('qty').value<{{$product->stock}})document.getElementById('qty').value++">+</button>
                            </div>
                        </div>
                    </div>
                    <button type="submit" class="primaryButton w100" style="padding:1.2rem;font-size:1.1rem;">Thêm Vào Giỏ
                        Hàng</button>
                </form>
            </div>
        </div>
                ...
                <div class="productGrid">
                    @foreach($relatedProducts as $related)
                        <div class="productCard">
                            <a href="{{route('products.show', $related->id)}}" class="productImage"
