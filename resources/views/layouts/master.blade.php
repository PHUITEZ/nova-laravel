...
<body>
    ...
    <div class="mobileMenu" id="mobileMenu">
        ...
        <ul>
            ...
            @auth
                <li><a href="{{auth()->user()->role === 'admin' ? route('admin.dashboard') : '#'}}">{{auth()->user()->name}}</a>
                </li>
                <li><a href="#" onclick="event.preventDefault();document.getElementById('logoutForm').submit();">Đăng
                        Xuất</a></li>
    ...
    <header class="mainHeader">
        <a href="{{route('home')}}" class="navLink noDecor"
            ...
            <h1 style="margin:0;font-size:1.5rem;letter-spacing:-0.05em;color:var(--textMain);font-weight:400;">NOVA
            </h1>
        </a>
        <form action="{{route('products.index')}}" method="GET" style="flex:1;max-width:600px;margin:0 2rem;"
            class="desktopSearch">
            <input type="text" name="search" placeholder="Tìm Kiếm Sản Phẩm..."
                style="width:100%;padding:0.7rem 1rem;border:1px solid var(--border-light);border-radius:50px;background:var(--bg-body);font-family:inherit;font-size:0.9rem;">
        </form>
        ...
        <div class="desktopNav" style="display:flex;align-items:center;gap:2rem;">
            <nav class="navigation">
                <ul class="navMenu" style="gap:2rem;">
                    <li><a href="{{route('home')}}" class="navLink {{request()->routeIs('home') ? 'active' : ''}}">Trang
                            Chủ</a></li>
                    <li><a href="{{route('products.index')}}"
                            class="navLink {{request()->routeIs('products.*') ? 'active' : ''}}">Sản Phẩm</a></li>
                    <li><a href="{{route('cart.index')}}"
                            class="navLink {{request()->routeIs('cart.*') ? 'active' : ''}}">Giỏ Hàng</a></li>
                </ul>
            </nav>
            <div style="display:flex;align-items:center;gap:1rem;white-space:nowrap;">
                @auth
                    <a href="{{auth()->user()->role === 'admin' ? route('admin.dashboard') : '#'}}" class="primaryButton"
                    ...
                    <form action="{{route('logout')}}" method="POST" style="display:inline;">
                        ...
                        <button type="submit" class="navLink"
                            style="background:none;border:none;cursor:pointer;font-family:inherit;padding:0;">Đăng
                            Xuất</button>
                    </form>
                @else
                    <a href="{{route('login')}}" class="navLink">Đăng Nhập</a>
                    <a href="{{route('register')}}" class="primaryButton"
                        style="padding:0.6rem 1.2rem;font-size:0.85rem;">Đăng Ký</a>
                @endauth
            </div>
        </div>
    </header>
    ...
    <footer class="mainFooter">
        <div class="row" style="max-width:1200px;margin:0 auto 3rem;text-align:left;">
            <div class="col">
                ...
                <p style="color:var(--text-light);font-size:0.9rem;">Thương Hiệu Thời Trang Tối Giản Dành Cho Người Hiện
                    Đại! Chúng Tôi Cam Kết Mang Đến Những Sản Phẩm Chất Lượng Cao Với Mức Giá Hợp Lý!</p>
            </div>
            <div class="col">
            ...
            <div class="col">
                ...
                <p style="color:var(--text-light);font-size:0.9rem;">Đăng Ký Nhận Bản Tin Để Không Bỏ Lỡ Những Ưu Đãi
                    Mới Nhất</p>
            </div>
        </div>
