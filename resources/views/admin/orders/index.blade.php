...
<li>
<a href="{{route('admin.orders')}}" style="display:block;padding:0.9rem 1rem;background:var(--text-main);color:white;border-radius:8px;text-decoration:none;transition:all 0.3s;">Đơn Hàng</a>
</li>
<li>
...
<td style="padding:1rem;color:var(--text-light);font-size:0.9rem;">{{$order->createdAt ? $order->createdAt->format('d/m/Y H:i') : 'N/A'}}</td>
<td style="padding:1rem;color:var(--accent);">{{number_format($order->total)}} đ</td>
<td style="padding:1rem;">
<span class="badge {{$order->status == 'completed' ? 'badgeSuccess' : ($order->status == 'cancelled' ? 'badgeDanger' : 'badgeWarning')}}">
{{ucfirst($order->status)}}
</span>
</td>
<td style="padding:1rem;">
<a href="{{route('admin.orders.show',$order->id)}}" class="qtyBtn" style="width:auto;padding:0 1rem;font-size:0.85rem;text-decoration:none;display:inline-flex;align-items:center;justify-content:center;">Xem</a>
</td>
</tr>
@endforeach
</tbody>
