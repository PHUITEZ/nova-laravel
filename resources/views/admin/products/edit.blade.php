...
@section('content')
    <div class="row" style="gap:2rem;align-items:flex-start;">
        <div class="col" style="flex:0 0 250px;">
            <div style="background:var(--bg-card);border:1px solid var(--border-light);border-radius:12px;padding:2rem;position:sticky;top:120px;">
                <div style="text-align:center;padding-bottom:1.5rem;border-bottom:1px solid var(--border-light);margin-bottom:1.5rem;">
                    <div style="width:70px;height:70px;background:var(--text-main);margin:0 auto 1rem;display:flex;justify-content:center;align-items:center;color:white;font-size:1.8rem;border-radius:50%;">
        ...
        <div class="col" style="flex:1;">
            <h1 style="margin:0 0 2rem 0;font-size:2rem;">Chỉnh Sửa Sản Phẩm</h1>
                ...
                <form action="{{route('admin.products.update', $product->id)}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:1.5rem;">
                        <div>
                            ...
                            <input type="text" name="name" class="formInput" value="{{$product->name}}" required>
                        </div>
                        <div>
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:1.5rem;">
                        <div>
                            ...
                            <input type="number" name="price" class="formInput" value="{{$product->price}}" required>
                        </div>
                        <div>
                            <label style="display:block;margin-bottom:0.5rem;color:var(--text-light);font-size:0.9rem;">Số Lượng Trong Kho</label>
                            <input type="number" name="stock" class="formInput" value="{{$product->stock}}" required>
                        </div>
                    </div>

                    <div style="margin-bottom:1.5rem;">
                        <label style="display:block;margin-bottom:0.5rem;color:var(--text-light);font-size:0.9rem;">Hình Ảnh Sản Phẩm</label>
                        
                        <input type="file" name="image" class="formInput" accept="image/*">
                        
                        @if($product->image)
                            <div style="margin-top: 1rem; padding: 10px; border: 1px dashed var(--border-light); border-radius: 8px; width: fit-content;">
                                <p style="margin: 0 0 0.5rem 0; font-size: 0.8rem; color: var(--text-light);">Ảnh hiện tại:</p>
        
                                @if(str_contains($product->image, 'http'))
                                    <img src="{{ $product->image }}" alt="Ảnh cũ" style="height: 100px; border-radius: 4px;">
                                @else
                                    <img src="{{ asset('storage/' . $product->image) }}" alt="Ảnh cũ" style="height: 100px; border-radius: 4px;">
                                @endif
                            </div>
                        @endif
                    </div>
                    ...
                    <div style="margin-bottom:2rem;">
                        ...
                        <textarea name="description" class="formInput" rows="5" style="resize:vertical;">{{$product->description}}</textarea>
                    </div>
                    
                    <button type="submit" class="primaryButton w100" style="padding:1rem;">Cập Nhật Sản Phẩm</button>
                </form>
            </div>
        </div>
    </div>
